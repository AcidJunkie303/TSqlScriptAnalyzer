$targetFilePath = "C:\projects\TSqlScriptAnalyzer\src\src\DatabaseAnalyzer.Contracts.DefaultImplementations\SqlParsing\CustomSqlFragmentVisitor.generated.cs"
...
$visitorType = $allTypes `
    | Where-Object { $_.Name -eq "TSqlFragmentVisitor" -and $_.Namespace -eq "Microsoft.SqlServer.TransactSql.ScriptDom"} `
    | Select-Object -First 1 
$methods = $visitorType.GetMethods("Public,Instance") `
    | Where-Object {($_.IsAbstract -eq $false -and $_.IsVirtual -and ($_.Name -eq "Visit" -or $_.Name -eq "VisitExplicit"))} `
...
public class TrackingSqlFragmentVisitor : TSqlFragmentVisitor
{
...
foreach($method in $methods)
{
    ...
    $isVisitExplicit = $method.Name -eq "VisitExplicit"
    ...
    $methodCode = $isVisitExplicit `
        ? @"

    public sealed override void VisitExplicit($parameterTypeName $parameterName)
    {
...
"@
    ...
    public override void Visit($parameterTypeName $parameterName)
    {
