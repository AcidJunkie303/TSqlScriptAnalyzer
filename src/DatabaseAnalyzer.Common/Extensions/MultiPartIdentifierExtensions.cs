...
public static class MultiPartIdentifierExtensions
{
    ...
    public static (string? fourthLast, string? ThirdLast, string? SecondLast, string? Last) GetParts(this MultiPartIdentifier identifier)
    {
        ArgumentNullException.ThrowIfNull(identifier);

        return identifier.Count switch
        {
            1 => (null, null, null, identifier.Identifiers[0].Value!),
            2 => (null, null, identifier.Identifiers[0].Value, identifier.Identifiers[1].Value!),
            3 => (null, identifier.Identifiers[0].Value, identifier.Identifiers[1].Value, identifier.Identifiers[2].Value!),
            4 => (identifier.Identifiers[0].Value, identifier.Identifiers[1].Value, identifier.Identifiers[2].Value!, identifier.Identifiers[3].Value!),
            _ => default
        };
    }
