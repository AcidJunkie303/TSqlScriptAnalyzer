...
public sealed class TableExtractorTests
{
    ...
    public void Extract_WhenColumnContainsUniqueKeyword_ThenColumnHasIsUniqueSetToTrue_ThenIndexContainsUniqueIndex()
    {
        ...
        tables[0].Indices.Should().ContainEquivalentOf(
            new IndexInformation("MyDb", "dbo", "T1", "PK_T1", TableColumnIndexTypes.Clustered | TableColumnIndexTypes.PrimaryKey | TableColumnIndexTypes.Unique, ["Id"], FrozenSet<string>.Empty, null!, "main.sql"),
            static options => options.Excluding(static x => x.CreationStatement));
        tables[0].Indices.Should().ContainEquivalentOf(
            new IndexInformation("MyDb", "dbo", "T1", IndexName: null, TableColumnIndexTypes.Unique, ["Email"], FrozenSet<string>.Empty, null!, "main.sql"),
            static options => options.Excluding(static x => x.CreationStatement));
    }
